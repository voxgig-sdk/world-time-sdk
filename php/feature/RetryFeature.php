...
class WorldTimeRetryFeature extends WorldTimeBaseFeature
{
    ...
    public function init(WorldTimeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WorldTimeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WorldTimeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WorldTimeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
