...
class WorldTimeNetsimFeature extends WorldTimeBaseFeature
{
    ...
    private function _simulate(WorldTimeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(WorldTimeContext $ctx, array $applied): void
    {
        $client = $this->client;
