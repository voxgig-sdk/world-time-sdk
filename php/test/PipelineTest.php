...
{
    private static function utility(): WorldTimeUtility
    {
        return new WorldTimeUtility();
    }
    ...
    private static function ctx(array $over = []): WorldTimeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WorldTimeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WorldTimeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WorldTimeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WorldTimeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WorldTimeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WorldTimeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WorldTimeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WorldTimeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WorldTimeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WorldTimeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WorldTimeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new WorldTimeResponse([]),
            'result' => new WorldTimeResult([]),
        ]);
        [, $err] = WorldTimeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WorldTimeSpec([]),
            'response' => null,
            'result' => new WorldTimeResult([]),
        ]);
        [, $err] = WorldTimeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WorldTimeSpec(['step' => 's']),
            'response' => new WorldTimeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WorldTimeResult([]),
        ]);
        [, $err] = WorldTimeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WorldTimeSpec(['step' => 's']),
            'response' => new WorldTimeResponse(self::resp(200, ['v' => 1])),
            'result' => new WorldTimeResult([]),
        ]);
        [, $err] = WorldTimeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WorldTimeSpec(['step' => 's']),
            'response' => new WorldTimeResponse(self::resp(200, ['v' => 2])),
            'result' => new WorldTimeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WorldTimeMakeResponse::call($ctx);
    ...
    {
        $preset = new WorldTimeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WorldTimeSpec([]),
            'response' => new WorldTimeResponse([]),
            'result' => new WorldTimeResult([]),
        ...
        [$got, $err] = WorldTimeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WorldTimeResult([])]);
        [, $err] = WorldTimeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WorldTimeSpec([]), 'result' => null]);
        [, $err] = WorldTimeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WorldTimeSpec(['step' => 's']),
            'result' => new WorldTimeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WorldTimeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WorldTimeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WorldTimeSpec(['step' => 's']),
            'result' => new WorldTimeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WorldTimeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WorldTimeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WorldTimeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WorldTimeSpec([]),
            'result' => new WorldTimeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WorldTimeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WorldTimeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WorldTimeError('boom', 'boom');
        $utility->fetcher = function (WorldTimeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WorldTimeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WorldTimeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WorldTimeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WorldTimeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WorldTimeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WorldTimeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WorldTimeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WorldTimeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WorldTimeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WorldTimeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WorldTimeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WorldTimeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WorldTimeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WorldTimeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WorldTimeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WorldTimeResult(['ok' => false])]);
        $this->expectException(WorldTimeError::class);
        WorldTimeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WorldTimeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WorldTimeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WorldTimeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WorldTimeBaseFeature();
        $b = new WorldTimeBaseFeature();
        WorldTimeFeatureAdd::call($ctx, $a);
        WorldTimeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WorldTimeBaseFeature
    {
        $f = new WorldTimeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WorldTimeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WorldTimeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WorldTimeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WorldTimePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WorldTimeResponse(['headers' => null]),
            'result' => new WorldTimeResult([]),
        ]);
        WorldTimeResultHeaders::call($ctx);
