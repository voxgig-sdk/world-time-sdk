...
class TimezoneDirectTest extends TestCase
{
    public function test_direct_list_timezone(): void
    {
        $setup = timezone_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-timezone", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_timezone(): void
    {
        $setup = timezone_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-timezone", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
