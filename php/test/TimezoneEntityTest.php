...
class TimezoneEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = WorldTimeSDK::test(null, null);
        $ent = $testsdk->Timezone(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = WorldTimeSDK::test($seed, null);
        $seen = iterator_to_array($base->Timezone(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = WorldTimeConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = WorldTimeSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Timezone(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = timezone_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "timezone." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WORLDTIME_TEST_TIMEZONE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $timezone_ref01_data = null;
        if (count($timezone_ref01_data_raw) > 0) {
...
function timezone_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/timezone/TimezoneTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["timezone01", "timezone02", "timezone03"] as $k) {
    ...
    $entid_env_raw = getenv("WORLDTIME_TEST_TIMEZONE_ENTID");
