...
class WorldTimeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WorldTimeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WorldTimeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WorldTimeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WorldTimeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WorldTimeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WorldTimeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WorldTimeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WorldTimeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WorldTimeError($op . "_allow",
                "WorldTimeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WorldTimeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WorldTimeError("graphql_error",
                "WorldTimeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Timezone($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_timezone === null) {
                $this->_timezone = new TimezoneEntity($this, null);
            }
            return $this->_timezone;
        }
        return new TimezoneEntity($this, $data);
    }
