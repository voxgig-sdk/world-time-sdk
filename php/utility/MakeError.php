...
// WorldTime SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class WorldTimeMakeError
{
    public static function call(?WorldTimeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WorldTimeContext([], null);
        }
        $op = $ctx->op ?? new WorldTimeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WorldTimeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WorldTimeError) ? $err->msg : (string)$err;
        $msg = "WorldTimeSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new WorldTimeError('', $msg, $ctx);
        ...
        if ($err instanceof WorldTimeError) {
