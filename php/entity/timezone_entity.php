...
class TimezoneEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TimezoneEntity($this->_client, $opts);
    }

    /**
     * @param Timezone|array $args Timezone data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WorldTimeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Timezone|array The current Timezone data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Timezone fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WorldTimeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Timezone fields).
     */
     ...
     *
     * @param TimezoneLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TimezoneLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Timezone|array The loaded Timezone as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WorldTimeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param TimezoneListMatch|array|null $reqmatch Match filter (any subset
     *   of Timezone fields) as an assoc-array; TimezoneListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Timezone[]|array A list of Timezone items as assoc-arrays at
     *   the SDK boundary; throws WorldTimeError on failure (item-5 convention).
     */
