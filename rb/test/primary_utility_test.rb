...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = WorldTimeBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = WorldTimeSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = WorldTimeSDK.test(nil, nil)
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = WorldTimeSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = WorldTimeSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = WorldTimeSDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = WorldTimeResult.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = WorldTimeOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = WorldTimeSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = WorldTimeContext.new(ctxmap, nil)
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = WorldTimeResponse.new(resp_map)
      if resp_map["body"]
