...
class TimezoneEntity
  ...
  def make
    opts = @_entopts.dup
    TimezoneEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WorldTimeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WorldTimeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Timezone.load works with no args).
  ...
  #
  # @param reqmatch [TimezoneListMatch, Hash, nil] match filter (any subset of
  #   Timezone fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
