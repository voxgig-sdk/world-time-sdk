...
class WorldTimeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WorldTimeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WorldTimeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WorldTimeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WorldTimeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = WorldTimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WorldTimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WorldTimeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WorldTimeError.new(
        "graphql_error", "WorldTimeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
