...
//     (\`new WorldTimeSDK(...)\` / \`WorldTimeSDK.test(...)\` become
//     \`WorldTimeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"ipn":{"test01":{"id":"test01"}},"ipn2":{"test01":{"id":"test01"}},"timezone":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WorldTimeSDK(...)\` and any
// \`WorldTimeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WorldTimeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WorldTimeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
