...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.WORLDTIME_TEST_LIVE ||
    'TRUE' === process.env.WORLDTIME_TEST_OVERRIDE
  ) {
  ...
  m.WORLDTIME_TEST_EXPLAIN = process.env.WORLDTIME_TEST_EXPLAIN || m.WORLDTIME_TEST_EXPLAIN
