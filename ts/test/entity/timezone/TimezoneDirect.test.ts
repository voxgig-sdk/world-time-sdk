...
describe('TimezoneDirect', async () => {
  ...
  test('direct-load-timezone', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-timezone', setup.live)) return
  ...
  test('direct-list-timezone', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-timezone', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WORLD_TIME_TEST_TIMEZONE_ENTID']
