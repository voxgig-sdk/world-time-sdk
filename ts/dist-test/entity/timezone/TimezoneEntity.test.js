...
(0, node_test_1.describe)('TimezoneEntity', async () => {
    ...
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.WorldTimeSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'timezone.' + op, live))
                return;
        }
        ...
        const timezone_ref01_match = {};
        const timezone_ref01_list = (await timezone_ref01_ent.list(timezone_ref01_match)).map((e) => e.data());
        // LOAD
        ...
        (0, node_assert_1.default)(timezone_ref01_data_dt0.id === timezone_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    let client = __1.WorldTimeSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['timezone01', 'timezone02', 'timezone03', 'timezone01', 'timezone02', 'timezone03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        client = new __1.WorldTimeSDK(merge([
