...
class FtCtrl extends WorldTimeControl
{
...
class FtClient
{
    ...
    private WorldTimeUtility $utility;

    public function __construct(WorldTimeUtility $utility, array $options)
    {
    ...
    public function get_utility(): WorldTimeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WorldTimeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WorldTimeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new WorldTimeUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WorldTimeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WorldTimeConfig::make_feature($name);
    ...
    private static function buildUrl(WorldTimeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WorldTimeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WorldTimeResponse($fetched);
        }
    ...
    private function fail(WorldTimeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WorldTimeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
