...
module WorldTimeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WorldTimeResponse.new({})
    ...
    if fetch_err
      ...
      response = WorldTimeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WorldTimeResponse.new(fetched)
    else
