...
module WorldTimeUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = WorldTimeContext.new({}, nil)
    end
    op = ctx.op || WorldTimeOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || WorldTimeResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(WorldTimeError) ? err.msg : err.to_s
    ...
    sdk_err = WorldTimeError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(WorldTimeError)
