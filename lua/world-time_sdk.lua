...
local WorldTimeSDK = {}
WorldTimeSDK.__index = WorldTimeSDK
...
WorldTimeSDK._make_feature = _make_feature


function WorldTimeSDK.new(options)
  local self = setmetatable({}, WorldTimeSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WorldTimeSDK:options_map()
...
function WorldTimeSDK:get_utility()
  return Utility.copy(self._utility)
end


function WorldTimeSDK:get_root_ctx()
  return self._rootctx
end


function WorldTimeSDK:prepare(fetchargs)
...
function WorldTimeSDK:direct(fetchargs)
...
-- by passing it.
function WorldTimeSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WorldTimeSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WorldTimeSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WorldTimeSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WorldTimeSDK.new(sdkopts)
  sdk.mode = "test"
