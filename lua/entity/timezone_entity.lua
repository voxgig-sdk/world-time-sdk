...
local TimezoneEntity = {}
TimezoneEntity.__index = TimezoneEntity


function TimezoneEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TimezoneEntity)
  self._name = "timezone"
  self._client = client
...
function TimezoneEntity:get_name()
  return self._name
end


function TimezoneEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TimezoneEntity.new(self._client, opts)
end
...
function TimezoneEntity:mark_deleted()
  self._deleted = true
end


function TimezoneEntity:deleted()
  return true == self._deleted
end


function TimezoneEntity:data_set(args)
  if args ~= nil then
...
function TimezoneEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TimezoneEntity:match_set(args)
  if args ~= nil then
...
function TimezoneEntity:match_get()
...
function TimezoneEntity:stream(action, args, callopts)
...
function TimezoneEntity:_run_op(ctx, post_done)
  local utility = self._utility
