...
describe("TimezoneDirect", function()
  it("should direct-list-timezone", function()
    local setup = timezone_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-timezone", setup.live and "live" or "unit")
  ...
  it("should direct-load-timezone", function()
    local setup = timezone_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-timezone", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function timezone_direct_setup(mockres)
