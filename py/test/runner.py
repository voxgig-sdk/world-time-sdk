...
class WorldTimeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WorldTimeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WorldTimeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WorldTimeTestRunner.getenv("WORLD_TIME_TEST_LIVE")
        override = WorldTimeTestRunner.getenv("WORLD_TIME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WorldTimeTestRunner.getenv(key)
        ...
        explain = WorldTimeTestRunner.getenv("WORLD_TIME_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WorldTimeTestRunner._test_control is not None:
            return WorldTimeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WorldTimeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WorldTimeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WorldTimeTestRunner.load_test_control()
...
def load_env_local():
    WorldTimeTestRunner.load_env_local()


def env_override(m):
    return WorldTimeTestRunner.env_override(m)


def entity_data(v):
    return WorldTimeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WorldTimeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WorldTimeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WorldTimeTestRunner.load_test_control()


def live_client_options():
    return WorldTimeTestRunner.live_client_options()


def live_delay_ms():
    return WorldTimeTestRunner.live_delay_ms()
