...
class TestTimezoneEntity:

    def test_should_create_instance(self):
        testsdk = WorldTimeSDK.test(None, None)
        ent = testsdk.Timezone(None)
    ...
    def test_should_stream(self):
        ...
        base = WorldTimeSDK.test(seed, None)
        seen = list(base.Timezone(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from worldtime_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = WorldTimeSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Timezone(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _timezone_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "timezone." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WORLD_TIME_TEST_TIMEZONE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        timezone_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.timezone")))
        timezone_ref01_data = None
        if len(timezone_ref01_data_raw) > 0:
...
def _timezone_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/timezone/TimezoneTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("WORLD_TIME_TEST_TIMEZONE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("WORLD_TIME_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = WorldTimeSDK(helpers.to_map(merged_opts))

    _live = env.get("WORLD_TIME_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WORLD_TIME_TEST_EXPLAIN") == "TRUE",
        "live": _live,
