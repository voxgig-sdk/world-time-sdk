...
from worldtime_sdk.worldtime_types import (
    Timezone,
    TimezoneLoadMatch,
    TimezoneListMatch,
)
...
class TimezoneEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TimezoneEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Timezone:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Timezone().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Timezone]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Timezone().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
