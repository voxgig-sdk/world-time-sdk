...
MCP server exposing the WorldTime SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `world-time_list` | `entity`, optional `query` map | First page of records as JSON |
| `world-time_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `world-time_list` and
`world-time_load` tools become available in new sessions.
