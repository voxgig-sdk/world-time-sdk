...
[MCP](https://modelcontextprotocol.io) server exposing the WorldTime SDK as
two agent tools — `world-time_list` and `world-time_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/world-time-mcp)
make build
...
claude mcp add --scope user world-time \
  -- /absolute/path/to/world-time-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./world-time-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// world-time_list: first page of records
{ "entity": "timezone" }
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/world-time-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user world-time \
     -- "$PWD"/dist/*/world-time-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `world-time_list` and `world-time_load` tools now appear
   in new sessions. Ask the agent to *"list timezone using world-time"*
   and it calls `world-time_list` with `{"entity":"timezone"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `world-time_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `world-time_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WORLD_TIME_APIKEY` | API key sent with every request. |
| `WORLD_TIME_BASE` | Optional override of the API base URL. |
...
```sh
./world-time-mcp -transport http -addr :18080 &
