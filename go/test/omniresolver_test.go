...
import (
	...
	omni "github.com/voxgig-sdk/world-time-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WorldTimeError); is {
				out := map[string]any{
					"name":    "WorldTimeError",
...
func omniCtx(arg any, client *sdk.WorldTimeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WorldTimeSDK); is {
			client = live
