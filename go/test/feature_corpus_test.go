...
//
// NewWorldTimeSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.WorldTimeSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewWorldTimeSDK(opts)
}
...
func fcCandidates(client *sdk.WorldTimeSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.WorldTimeSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.WorldTimeSDK, name string) any {
