...
func TestTimezoneEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Timezone(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Timezone(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		timezoneRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.timezone")))
...
func timezoneBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WORLD_TIME_TEST_TIMEZONE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WORLD_TIME_TEST_TIMEZONE_ENTID"])
